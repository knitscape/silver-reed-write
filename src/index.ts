...
import { drawBitmapToCanvas, getRow } from "./bitmap";
...
function drawBitmap() {
  const canvas = document.getElementById("pattern-canvas") as HTMLCanvasElement;
  drawBitmapToCanvas(canvas, GLOBAL_STATE.basePattern);
  canvas.style.width = `${GLOBAL_STATE.basePattern.width * 20}px`;
  canvas.style.height = `${GLOBAL_STATE.basePattern.height * 20}px`;
  if ("currentRow" in GLOBAL_STATE.interactionMode) {
    highlightRow(GLOBAL_STATE.interactionMode.currentRow);
  }
}
...
async function processState(jsonData) {
  ...
  if ("currentRow" in GLOBAL_STATE.interactionMode) {
    ...
    GLOBAL_STATE.interactionMode.currentRow =
      GLOBAL_STATE.interactionMode.currentRow + 1;
    if (
      GLOBAL_STATE.interactionMode.currentRow >= GLOBAL_STATE.basePattern.height
    ) {
      GLOBAL_STATE.interactionMode.currentRow = 0; // wrap around to the first row
    }

    await writePatternRow(GLOBAL_STATE.interactionMode.currentRow); // write the current row to the machine
  }
...
function knittingUI(knittingModeState: KnittingMode) {
  const height = GLOBAL_STATE.basePattern.height;
  const currentRow = knittingModeState.currentRow;

  return html` <div class="flex flex-col h-full gap-2 bg-base-300">
    ${patternConfig(GLOBAL_STATE.patternConfig)}
    ...
    <div class="bg-base-200 outline-1 outline-black">
      ...
      <button
        @click=${() => writePatternRow(knittingModeState.currentRow)}
        ?disabled=${!serial.connected()}
        class="btn btn-xs btn-info">
        Knit!
      </button>
      <span
        >Side: ${knittingModeState.carriageSide} Row:
        ${knittingModeState.currentRow}</span
      >
...
function mainUI(interactionMode: InteractionMode) {
  if ("currentRow" in interactionMode) {
    return knittingUI(interactionMode);
  } else {
    return designUI(interactionMode);
  }
}
...
function view() {
  return html`
    <div class="flex flex-col h-screen">
      ${toolbar()} ${mainUI(GLOBAL_STATE.interactionMode)}
    </div>
  `;
}
